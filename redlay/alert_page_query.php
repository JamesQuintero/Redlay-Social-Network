...
if($num==1)
{
    ...
    if($page>0)
    {
        $query=mysql_query("SELECT alert_user_ids, alert_timestamps, alert_information, alerts_read, new_alerts, alert_ids FROM alerts WHERE user_id=$_SESSION[id] LIMIT 1");
                            ...
                            if($query&&mysql_num_rows($query)==1)
                            {
                                ...
                                if($index!=-1)
                                {
                                    if($text[$x][0]=="picture_comment"||$text[$x][0]=="picture_like"||$text[$x][0]=="picture_dislike")
                                        $text[$x][]="https://s3.amazonaws.com/bucket_name/users/$_SESSION[id]/thumbs/".$pictures[$index].".".$image_types[$index]; //AWS S3 bucket url
                                    else if($text[$x][0]=="liked_picture_comment"||$text[$x][0]=="disliked_picture_comment"||$text[$x][0]=="comment_same_picture")
                                        $text[$x][]="https://s3.amazonaws.com/bucket_name/users/".$text[$x][2]."/thumbs/".$pictures[$index].".".$image_types[$index]; //AWS S3 bucket url
                                }
                            }
                        }
...
else if($num==2)
{
    ...
    if($query&&mysql_num_rows($query)==1)
    {
        ...
        log_error("alert_page_query.php: (1): ", mysql_error());
}

//deletes all alerts
else if($num==3)
{
//    $query=mysql_query("SELECT alert_ids, alert_user_ids, alert_timestamps, alert_information, alerts_read, new_alerts FROM alerts WHERE user_id=$_SESSION[id] LIMIT 1");
//    if($query&&mysql_num_rows($query)==1)
//    {
//        $array=mysql_fetch_row($query);
//        $alert_ids=explode('|^|*|', $array[0]);
//        $alert_user_ids=explode('|^|*|', $array[1]);
//        $alert_timestamps=explode('|^|*|', $array[2]);
...
//
//        $num_unread_alerts_deleted=0;
//        for($x = 0; $x < sizeof($alert_ids); $x++)
//        {
//            if(!in_array($alert_ids[$x], $alerts))
//            {
//                $temp_alert_ids[]=$alert_ids[$x];
...
//
//        $new_alerts-=$num_unread_alerts_deleted;
//        for($x = 0; $x < sizeof($alerts_read); $x++)
//        {
//            if($alerts_read[$x]==1)
//                $temp_new_alerts++;
//        }
//
//        $query=mysql_query("UPDATE alerts SET alert_ids='$alert_ids', alert_user_ids='$alert_user_ids', alert_timestamps='$alert_timestamps', alert_information='$alert_information', alerts_read='$alerts_read', new_alerts=$new_alerts WHERE user_id=$_SESSION[id]");
//        if($query)
//            echo "Alerts deleted";
//        else
//            echo "Something went wrong.";
//    }
      $query=mysql_query("UPDATE alerts SET alert_ids='', alert_user_ids='', alert_timestamps='', alert_information='', alerts_read='', new_alerts='' WHERE user_id=$_SESSION[id] LIMIT 1");
      if($query)
         echo "Alerts deleted";
      else
      {

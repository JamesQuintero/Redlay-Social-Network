...
//        $sort=1;
        ...
        $privacy=get_user_privacy_settings($ID);
        if($privacy[1][2]=='yes'||(isset($_SESSION['id'])&&$ID==$_SESSION['id'])||(isset($_SESSION['id'])&&user_is_friends($ID, $_SESSION['id'])=='true'))
        {
            if(is_valid_month($month)||$month=="all")
            {
               ...
               $user_audiences=get_audience_current_user($ID);
                ...
                $query=mysql_query("SELECT post_ids, posts, user_ids_posted, comments, likes, dislikes, timestamps, post_groups, comments_user_id, comment_ids, comment_likes, comment_dislikes, comment_timestamps FROM content WHERE user_id=$ID LIMIT 1");
                if($query&&mysql_num_rows($query)==1)
                {
                    $array=mysql_fetch_row($query);
                    $post_ids=explode('|^|*|', $array[0]);
                    ...
                    $dislikes=explode('|^|*|', $array[5]);
                    $timestamps=explode('|^|*|', $array[6]);
                    $audiences=explode('|^|*|', $array[7]);
                    $comments_users_sent=explode('|^|*|', $array[8]);
                    $comment_ids=explode('|^|*|', $array[9]);
                    $comment_likes=explode('|^|*|', $array[10]);
                    $comment_dislikes=explode('|^|*|', $array[11]);
                    $comment_timestamps=explode('|^|*|', $array[12]);
                    ...
                    {
                        if(!user_id_terminated($user_ids_posted[$x]))
                        {
...
//                    print_r($audiences);
                    ...
                    for($x=0; $x < sizeof($post_ids); $x++)
                    {
                        ...
                        if((  $month=="all"||$temp_timestamps[$x][0]==$month  )&&(  $year=="all"||$temp_timestamps[$x][2]==$year  )&&(  $phrase=='none'||strpos($posts[$x], $phrase)!==false  )&&(can_view($user_audiences, $audiences[$x])||(isset($_SESSION['id'])&&$ID==$_SESSION['id'])))
                        {
                            $temp_post_ids[]=$post_ids[$x];
                            $temp_posts[]=$posts[$x];
                            $temp_user_ids_posted[]=$user_ids_posted[$x];
...
//                    print_r($post_ids);
                    ...
                    for($x =0; $x < sizeof($post_ids); $x++)
                    {
                        ...
                        for($y = 0; $y < sizeof($likes[$x]); $y++)
                        {
                            if(isset($_SESSION['id'])&&$likes[$x][$y]==$_SESSION['id'])
                                $liked=true;
                        }
                        $has_liked[$x]=$liked;
                        ...
                        for($y = 0; $y < sizeof($dislikes[$x]); $y++)
                        {
                            if(isset($_SESSION['id'])&&$dislikes[$x][$y]==$_SESSION['id'])
                                $disliked=true;
                        }
...
//                    print_r($comment_timestamps);
                    ...
                    if($total_size>0)
                    {
                        $JSON=array();
                        $JSON['names']=$names;
                        $JSON['users_sent']=$user_ids_posted;
                        $JSON['profile_pictures']=$profile_pictures;
                        $JSON['posts']=$posts;
...
else if($num==5)
{
    ...
    if(is_id($ID)&&user_id_exists($ID)&&!user_id_terminated($ID))
    {
        ...
        if($privacy[2][3]=='yes'||(isset($_SESSION['id'])&&$ID==$_SESSION['id'])||(isset($_SESSION['id'])&&user_is_friends($ID, $_SESSION['id'])))
        {
            if($number==1||$number==2)
            {
                $query=mysql_query("SELECT pictures, image_audiences, image_types, timestamp FROM pictures WHERE user_id=$ID LIMIT 1");
                if($query&&mysql_num_rows($query)==1)
                {
                    $array=mysql_fetch_row($query);
                    $pictures=explode('|^|*|', $array[0]);
                    ...
                    $image_timestamps=explode('|^|*|', $array[3]);
    ...
    if(is_id($ID)&&user_id_exists($ID)&&!user_id_terminated($ID))
    {
        ...
        if($privacy[1][0]=='yes'||(isset($_SESSION['id'])&&$ID==$_SESSION['id']))
        {
            ...
            if($query&&mysql_num_rows($query)==1&&$query2&&mysql_num_rows($query2)==1&&$query3&&mysql_num_rows($query3)==1&&$query5&&mysql_num_rows($query5)==1&&$query6&&mysql_num_rows($query6)==1)
            {
                ...
                if($array5['videos']!='')
                    $JSON['num_videos']=sizeof(explode('|^|*|', $array5['videos']));
                else
                    $JSON['num_videos']=0;
...
else if($num==11)
{
    ...
    {
        $privacy=get_user_privacy_settings($ID);
        if($privacy[1][7]=='yes'||(isset($_SESSION['id'])&&$ID==$_SESSION['id'])||(isset($_SESSION['id'])&&user_is_friends($ID, $_SESSION['id'])))
        {
...
else if($num==15)
{
    ...
    if(is_id($ID)&&user_id_exists($ID)&&!user_id_terminated($ID))
    {
        ...
        $privacy=get_user_privacy_settings($ID);
        if($privacy[1][0]=='yes'||(isset($_SESSION['id'])&&$ID==$_SESSION['id'])||(isset($_SESSION['id'])&&user_is_friends($ID, $_SESSION['id'])))
        {
            ...
            if($query&&mysql_num_rows($query)==1)
            {
                ...
                {
                    $array2=mysql_fetch_row($query2);
                    $date_joined=get_time_since($array2[0], $timezone);
                }
